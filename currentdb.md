...
CREATE TABLE public.attendance_evidence (
  ...
  qr_token_id text,
  device_integrity_result text CHECK (device_integrity_result IS NULL OR (device_integrity_result = ANY (ARRAY['pass'::text, 'fail'::text, 'mock'::text]))),
...
CREATE TABLE public.employees (
  ...
  shift_id text,
  pay_frequency text CHECK (pay_frequency IS NULL OR (pay_frequency = ANY (ARRAY['monthly'::text, 'semi_monthly'::text, 'bi_weekly'::text, 'weekly'::text]))),
  work_days ARRAY,
  whatsapp_number text,
  preferred_channel text CHECK (preferred_channel IS NULL OR (preferred_channel = ANY (ARRAY['email'::text, 'whatsapp'::text, 'sms'::text, 'in_app'::text]))),
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
