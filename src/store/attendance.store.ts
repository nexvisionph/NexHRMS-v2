 ...
 * Uses fixed +8h offset — never relies on browser timezone.
 */
function toLocalPHT(utcISO: string): string {
    const ms = new Date(utcISO).getTime() + 8 * 60 * 60 * 1000;
    const pht = new Date(ms);
    ...
    persist(
        (set, get) => ({
            ...
            getEventsForDate: (date) =>
                get().events.filter((e) => toLocalPHT(e.timestampUTC) === date),
                ...
                set((s) => {
                    ...
                    for (const empId of employeeIds) {
                        const dayEvents = s.events.filter(
                            (e) => e.employeeId === empId && toLocalPHT(e.timestampUTC) === date
                        );
            ...
            autoMarkAbsentAfterShift: (date, employees) => {
                ...
                for (const emp of employees) {
                    ...
                    const hasCheckIn = state.events.some(
                        (e) => e.employeeId === emp.id && e.eventType === "IN" && toLocalPHT(e.timestampUTC) === date
                    );
            ...
            resetTodayLog: (employeeId) => {
                ...
                set((s) => ({
                    ...
                    events: s.events.filter((e) => {
                        ...
                        return ts ? toLocalPHT(ts) !== today : true;
                    }),
                }));
            },
