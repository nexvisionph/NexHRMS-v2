                        ...
                        {editingLog && (
                            <div className="space-y-4 pt-2">
                                ...
                                <div className="grid grid-cols-3 gap-3">
                                    <div><label className="text-xs font-medium text-muted-foreground">Late Min</label><Input type="number" min="0" max="480" value={ovLate} onChange={(e) => setOvLate(e.target.value)} placeholder="0" className="mt-1 h-9" /></div>
                                    <div><label className="text-xs font-medium text-muted-foreground">Hours</label><div className="mt-1 h-9 flex items-center justify-center rounded-md border bg-muted/30 text-sm font-mono">{(() => { if (!ovCheckIn || !ovCheckOut) return "—"; const inP = ovCheckIn.split(":"); const outP = ovCheckOut.split(":"); const inD = Number(inP[0]) + Number(inP[1] || 0) / 60; const outD = Number(outP[0]) + Number(outP[1] || 0) / 60; const effIn = Math.max(inD, 8); const hrs = Math.max(0, outD - effIn - 1); return `${Math.round(hrs * 100) / 100}h`; })()}</div></div>
                                    <div><label className="text-xs font-medium text-amber-600 dark:text-amber-400">OT</label><div className="mt-1 h-9 flex items-center justify-center rounded-md border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/20 text-sm font-mono font-semibold text-amber-700 dark:text-amber-400">{(() => { if (!ovCheckIn || !ovCheckOut) return "—"; const inP = ovCheckIn.split(":"); const outP = ovCheckOut.split(":"); const inD = Number(inP[0]) + Number(inP[1] || 0) / 60; const outD = Number(outP[0]) + Number(outP[1] || 0) / 60; const effIn = Math.max(inD, 8); const hrs = Math.max(0, outD - effIn - 1); const ot = Math.max(0, hrs - 8); return ot > 0 ? `+${Math.round(ot * 100) / 100}h` : "0h"; })()}</div></div>
                                </div>
                                <div><label className="text-sm font-medium">OT Description</label><Input value={ovOtDescription} onChange={(e) => setOvOtDescription(e.target.value)} placeholder="e.g. Extended site visit, Emergency overtime" className="mt-1" /></div>
