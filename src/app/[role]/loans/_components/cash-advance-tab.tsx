...
import { formatCompanyLoanType } from "@/app/[role]/loans/_lib/government-loans";
...
export function CashAdvanceTab() {
    ...
    }, [loans, statusFilter, search, employees]);
    ...
    const stats = useMemo(() => {
        ...
        return {
            ...
            totalSettled: loans.filter((l) => l.status === "settled").length,
        };
    }, [loans]);
        ...
        try {
            ...
            if (emp) {
                dispatchNotification("loan_created", {
                    name: emp.name,
                    type: formatCompanyLoanType(formType),
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                ...
                <Dialog open={open} onOpenChange={setOpen}>
                    <DialogTrigger asChild>
                        <Button className="gap-1.5"><Plus className="h-4 w-4" /> Create Loan</Button>
                    </DialogTrigger>
                    <DialogContent className="max-w-md">
                        ...
                        <div className="space-y-4 pt-2">
                            ...
                            <div>
                                ...
                                <Select value={formType} onValueChange={setFormType}>
                                    ...
                                    <SelectContent>
                                        ...
                                        <SelectItem value="salary_loan">Company Loan</SelectItem>
                            ...
                            {formAmount && formMonthly && Number(formMonthly) > 0 && (
                                <p className="text-xs text-muted-foreground">≈ {Math.ceil(Number(formAmount) / Number(formMonthly))} monthly installments</p>
                            )}
                            <div>
                            ...
                            <div>
                                ...
                                <p className="text-[10px] text-muted-foreground mt-0.5">Maximum % of net pay that can be deducted (default 30%)</p>
                            </div>
                                                        ...
                                                        <div className="flex items-center gap-1">
                                                                ...
                                                                <>
                                                                    ...
                                                                    <Button variant="ghost" size="icon" className="h-7 w-7 text-emerald-600" onClick={() => {
                                                                        ...
                                                                        try {
                                                                            ...
                                                                            if (emp) dispatchNotification("loan_settled", { name: emp.name, type: formatCompanyLoanType(loan.type) }, emp.id, emp.email ?? undefined, emp.phone, undefined, { suppressToast: true });
                                                                        } catch { /* best-effort */ }
                                                                    }} title="Settle fully">
                                                                    ...
                                                                    <Button variant="ghost" size="icon" className="h-7 w-7 text-amber-600" onClick={() => {
                                                                        ...
                                                                        try {
                                                                            const emp = employees.find((e) => e.id === loan.employeeId);
                                                                            if (emp) dispatchNotification("loan_frozen", { name: emp.name, type: formatCompanyLoanType(loan.type) }, emp.id, emp.email ?? undefined, emp.phone, undefined, { suppressToast: true });
                                                                        } catch { /* best-effort */ }
                                                                    }} title="Freeze">
                                                            ...
                                                            {loan.status === "frozen" && (
                                                                <Button variant="ghost" size="icon" className="h-7 w-7 text-blue-600" onClick={() => {
                                                                    ...
                                                                    try {
                                                                        const emp = employees.find((e) => e.id === loan.employeeId);
                                                                        if (emp) dispatchNotification("loan_unfrozen", { name: emp.name, type: formatCompanyLoanType(loan.type) }, emp.id, emp.email ?? undefined, emp.phone, undefined, { suppressToast: true });
                                                                    } catch { /* best-effort */ }
                                                                }} title="Unfreeze">
                                    ...
                                    <Card key={loan.id} className="border border-border/50">
                                        <CardContent className="p-4">
                                            <div className="flex items-center justify-between mb-3">
                                                <div>
                                                    <p className="text-sm font-semibold">{getEmpName(loan.employeeId)} — {formatCompanyLoanType(loan.type)}</p>
                                            ...
                                            <div className="overflow-x-auto">
                                                <Table>
                                                    ...
                                                    <TableBody>
                                                        {schedule.length === 0 ? (
                                                            <TableRow><TableCell colSpan={4} className="text-center text-sm text-muted-foreground py-4">No schedule generated</TableCell></TableRow>
                                                            ...
                                                            <TableRow key={idx}>
                                                                ...
                                                                <TableCell>
                                                                    <Badge variant={inst.paid ? "default" : "secondary"} className={`text-[10px] ${inst.paid ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : ""}`}>
                                                                        {inst.paid ? "paid" : inst.skippedReason ? `skipped (${inst.skippedReason})` : "pending"}
                                                                    </Badge>
                                                                </TableCell>
                                                            </TableRow>
                                                        ))}
